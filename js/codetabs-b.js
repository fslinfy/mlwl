function codeTabs(tabData) {
  ...
  for (var i = 0; i < tabData.length; i++) {
    ...
    if (tabData[i].name === "Description") {
      div = $(
        "<div role='tabpanel' class='tab-pane' style='height:350px; width:780px; overflow-y:scroll; font-size: 150%;'>"
      );
    } else {
      div = $(
        "<div role='tabpanel' class='tab-pane' style='height:350px; width:780px; overflow-y:scroll; font-size: 135%;'>"
      );
    }
    if (i === 0) {
      div.addClass("active");
  ...
  $("<span><br/>Switch theme:<br/><span><div id='switcher'></div>").appendTo(
    "#codetabs-" + tabData.length
  );
  addThemeTab();
}
...
function getFileText(url, divID, /*lang*/ tabName) {
  $.ajax({
    ...
    success: function (data) {
      ...
      tabName === "Description"
        ? preTag.html(strippedData)
        : preTag.text(strippedData);
      $("#" + divID).append(preTag);
      if (tabName !== "Description") {
        prettyPrint();
      }
...
function removeCodeTabsCode(html) {
  ...
  if (beginIndex > -1) {
    var blockToRemove = html.substr(
      beginIndex,
      endIndex - beginIndex + endBlock.length
    );
