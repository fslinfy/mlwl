Ext.define("MyApp.view.main.type.TypeCtrl", {
  ...
  requires: ["MyApp.view.main.type.TypeView"],
  ...
  onFilterChange: function (v) {
    //return storeFilter(this.getView().getStore(),'T_name',v.rawValue);
    var store = this.getView().getStore();
    var regExp = new RegExp(".*" + v.rawValue + ".*");
    store.clearFilter();
    store.filterBy(function (record, id) {
      return (
        regExp.test(record.get("T_name")) || regExp.test(record.get("T_code"))
      );
    });
  },
});
