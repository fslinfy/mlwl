var that;
var cptzdmxStore;
var TzdckshsaveCallBack = function (the) {
...
Ext.define('MyApp.view.main.cptzdgl.CptzdckshCtrl', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.CptzdckshCtrl',
    requires: [
        'MyApp.view.main.cptzdgl.CptzdckshView',
        'MyApp.view.main.tree.WorkerSelectTree'
    ],
    locQuery: function (the) {
        console.log("cptzd  cksh locquery ");
        var v = the.viewname.getViewModel();
        var khid = v.get('khid');
        var ckid = v.get('ckid');
        var store = cptzdmxStore;
        store.clearFilter();
        store.proxy.extraParams.loc = "cptzdmxcksh";
    ...
    init: function () {
        ...
        cptzdmxStore.on("load", function () {
            ...
            store.proxy.extraParams.khid = khid;
            store.proxy.extraParams.ckid = ckid;
            store.proxy.extraParams.loc = 'cptzdcksh';
            store.reload();
        });
 ...
 if (sys_customer_id > 0) {
    ...
    onCptzdshShowView: function (button) {
        ...
        // cptzdmx_store.proxy.extraParams.loc = "cptzdmxcksh";
   ...
   */
    ...
    onCptzdOkSave: function () {
        this.CptzdckshSave('ok', this);
    },
    onCptzdDeleteSave: function () {
        this.CptzdckshSave('delete', this);
    },
    CptzdckshSave: function (loc, the) {
        var p = the.lookupReference('popupmxShowWindow').getViewModel();
        var tzid = p.get('tzid');
        if (tzid == 0) {
            return;
        ...
        /*   var msg = "调账单号：" + p.get('tzdh') + "<br>客户名称：" + p.get('khmc');// + "<br>进库日期：" + p.get('tzrq');
           ...
           var abc = Ext.Msg.confirm(title, msg, function (e) {
               if (e == 'yes') {
                   AjaxDataSave('cptzdshsave', loc, tzid, TzdckshsaveCallBack, the);
               }
           })*/
        ...
        Ext.Msg.confirm(title, msg, function (e) {
            if (e == 'yes') {
                ...
                AjaxDataSave('cptzdckshsave', loc, encodedString, TzdckshsaveCallBack, the);
            }
        })
