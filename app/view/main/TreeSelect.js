...
function treeSelect(mc,the,kcbz,viewname,resh,callback)
{
    ...
    var url="";
    switch(mc){
        case "ckmc":url=sys_ActionPHP+"?act\x3dlocationselecttreelist";
            this.f_id="ckid";
            this.f_mc="ckmc";
            break;
        case "khmc":url=sys_ActionPHP+"?act\x3dcustomerselecttreelist";
            this.f_id="khid";
            this.f_mc="khmc";
            break;
        case "cpmc":
...
{   singleExpand:true,
    ...
    store:treestore,
    tbar:[{labelWidth:40,xtype:"triggerfield",fieldLabel:"\u8fc7\u6ee4",
    flex:1,triggerCls:"x-form-clear-trigger",
    onTriggerClick:function(){this.reset()},
    listeners:{change:function(){
        var tree=this.up("treepanel"),v,matches=0,v=new RegExp(this.getValue(),"i");
        Ext.suspendLayouts();
        tree.store.filter({filterFn:function(node){
            var children=node.childNodes,
            len=children&&children.length,
            visible=node.isLeaf()?v.test(node.get("text"))||v.test(node.get("py_code"))||v.test(node.get("code")):false,i;
            for(i=0;i<len&&!(
    ...
    bbar:{reference:"bbar",
    items:["-\x3e",{text:"\u786e\u8ba4",itemId:"btnTreeSelect",icon:"images/right.gif",
    disabled:true,
    handler:function(){onSelectOkClick()}
    },
    {
        text:"\u653e\u5f03",
