...
gridgroupingView ={};
         ...
         mycolModel= [
            {label: 'ID',  name: 'Id', index: 'Id', sorttype: "string",search: false ,hidden:true },
            { label: 'id', name: 'C_id', index: 'C_id', width:30, sorttype: "string",editable:false,
            search: true , searchoptions: { sopt:  ['eq']}},
            {label: '代码',  name: 'C_code', index: 'C_code', width:80, sorttype: "string",editable: true,
            searchoptions:{sopt:['cn','eq']} ,recreateFilter:false            
            },
            {label: '客户名称', name: 'C_name', index: 'C_name', width:200 , sorttype: "string",search: true,
            editable: true,editrules:{ required : true}, recreateFilter:true, searchoptions: { sopt:  ['cn','eq']} 
            },
            { label: '客户简称', name: 'C_shortname', index: 'C_shortname', width:100,searchoptions:{sopt:['cn','eq']}, sorttype: "string",editable: true,
            ...
            {label: '地址',  name: 'Address', index: 'Address', width:300, searchoptions:{sopt:['cn','eq']}, sorttype: "string",editable: true },
            {label: '联系电话',  name: 'Tel', index: 'Tel', searchoptions:{sopt:['cn','eq']}, sorttype: "string",editable: true 
             },
            {label: '移动电话',  name: 'smsphone', index: 'smsphone', searchoptions:{sopt:['cn','eq']},sorttype: "string",editable: true },
            {label: '有效日期',  name: 'Enddate', width: 150, search: true,sorttype : "date",
              searchoptions:{sopt:['eq'], dataInit:function (element) {
                                $(element).datepicker({dateFormat: 'yy-m-d',showOn: 'focus' });
                            }},
             editable: true, edittype:"text", editoptions: {dataInit:function (element) {
                                $(element).datepicker({dateFormat: 'yy-m-d',showOn: 'focus' });
                            }}
            },
            {label: '活跃',  name: 'Active', index: 'Active',align: 'center', width: 60, 
             formatter: "checkbox",sorttype: "string",
             editable: true, edittype: "checkbox", editoptions: {value: "1:0" },
             stype: "select", searchoptions: { sopt: ["eq"], value: ":;1:Yes;0:No" }
             },
            {label: '独立单价',  name: 'Aloneprice', index: 'Aloneprice',width: 60, align: 'center',
            formatter: "checkbox",sorttype: "string",
            editable: true, edittype: "checkbox", editoptions: {value: "1:0" },
            searchoptions: { sopt: ["eq"], value: ":;1:Yes;0:No", attr: {size:18, maxlength: 18, style: "width:130px;margin-top:1px;"}},
            stype: "select"
            }
            ];
