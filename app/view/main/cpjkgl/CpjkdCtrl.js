...
Ext.define("MyApp.view.main.cpjkgl.CpjkdCtrl", {
  ...
  onBtnNewClick: function (rs) {
    this.getView()
      .getStore()
      .addSorted([{ L_id: sys_location_id }]);
    return false;
  },
  ...
  init: function () {
    ...
    /*  cpjkd_store = Ext.create('Ext.data.Store', {
          ...
          */
    var store = this.getView().getStore();
    store.proxy.extraParams.allkh = 0;
    //store.reload();
  ...
  onUploadFile: function () {
    uploadWin = new Ext.Window({
      ...
      closeAction: "destroy",
      border: false,
      items: [{ xtype: "msFieldFileImageContainer" }],
    }).show();
  },
  onFilterChange: function (v) {
    ...
    store.filterBy(function (record, id) {
      return (
        ...
        regExp.test(record.get("C_shortname")) ||
  ...
  onCpjkdFormSubmit: function () {
    ...
    if (rq < sys_option_min_date) {
      Ext.MessageBox.alert(
        "注意！",
        "输入入库日期不能小于：" + sys_option_min_date
      );
      return false;
    }
    //return ;
