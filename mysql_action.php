...
//error_log("You messed up!", 3, "logs/my-errors.log");
$act = $_GET['act'];
if (!$act) {
	$act = $_POST['act'];
}
...
function enterpriselist()
{
	...
	return getjsonstoredata_new($sqlstr, 0);
}
function cktjjdlist()
{
	$ny = (int)$_GET['ny'];
...
function customerlist()
{
	...
	//return $allkh.'  '. $sqlstr;
	//$query = mysql_query($sqlstr);
	return getjsonstoredata_new($sqlstr, $totalrow);
}
...
function locationselecttreelist()
{
	$p_e_code = $_GET['p_e_code'];
	$loc = $_GET['loc'];
	$sqlstr = "SELECT L_code AS code, L_name AS text  ,L_id  AS id,option_min_date(l_id) as minrq ";
...
function systemsetting()
{
	...
	if ($appid == "1") {
		$sqlstr = "select a.L_id as id ,a.L_name as name,e.E_code,e.E_name,a.areas ,a.L_area from location a,enterprise e where  a.E_code=e.E_code and a.active=1 and  a.l_id=" . $systemid;
	} else {
...
function cpjkdmxlist_pc()
{
	...
	$filter .= " and cpjkd.khid=customer.c_id  AND cpjkd.L_id=location.L_id and location.active=1 and customer.active=1 ";
	if ($loc == "cpjkdprt") {
		$sqlstr = " (SELECT 
  '1' AS mxdh,
  m.jkid,
...
WHERE cpjkd.jkid = m.jkid AND m.mxid=cw.mxid  " . $filter;
		...
		$sqlstr = " (SELECT '1' as mxdh, m.jkid,m.mxid,0 as jeid,m.cdmc,m.cpmc,m.bzid,m.bzmc,m.cpgg,'' as cw,'' as cpph,m.jldw,m.jcsl,m.jczl,m.czdj,0 as jcje,0 as xjje 
     	,'' as gs,'' as byg,'' as cg ,'' as zyfs FROM cpjkdmx m,cpjkd ,customer,location  where cpjkd.jkid=m.jkid  " . $filter;
	}
	...
	$sqlstr = $sqlstr . " case when j.zljs=1 then j.sl else 0 end  as  jczl ,j.dj as czdj,case when j.xjbz=1 then 0 else j.je end  as  jcje ,case when j.xjbz=1 then j.je else 0 end  as  xjje 
		,gs,byg,cg ,'' as zyfs FROM cpjkdmx m,cpjkd,cpjkdje j,customer,location where cpjkd.jkid=m.jkid and m.mxid=j.mxid  " . $filter;
...
function cpckdmxlist_pc()
{
	...
	$filter .= " and cpxsd.khid=customer.c_id  AND cpxsd.L_id=location.L_id and location.active=1 and customer.active=1 ";
	$sqlfilter = $filter;
	if ($Lid > 0) {
	...
	$sqlstr = "(SELECT '1' AS mxdh, ck.ckid,ck.mxid,ck.cdmc,ck.cpmc,ck.bzmc,ck.cpgg,ck.cpph,ck.sm,ck.jldw,
		...
		'' as gs,'' as byg,'' as cg,0 as jeid,cm.ccsl,cm.cczl ,m.bzid,cpxsd.xjbz
  		FROM cpxsdmx m,cpckd,cpxsd,cpckdmx cm,location,customer   
	...
	$sqlstr .= " ) ck LEFT OUTER JOIN (SELECT 
      ...
      SUM(w.zl) AS cwzl 
    FROM 
    cpckdcw w ,cpckdmx m,cpckd,cpxsd ,location,customer 
...
function cpjkdlist_pc()
{
    ...
    FROM   CPJKDMX W,   cpjkd,location  ,cpjkdcw cw,customer
    WHERE cpjkd.jkid = w.jkid and w.mxid=cw.mxid " . $filter;
	} else {
		$sqlstr = "SELECT *  FROM CPJKD   LEFT OUTER JOIN  (
		SELECT  W.JKid AS id,SUM(w.JCsl) AS JCSl, SUM(w.JCzl) AS JCzl,  SUM(w.JCJE) AS JCJE 
        FROM CPJKDMX W,cpjkd,customer,location  where cpjkd.jkid=w.jkid " . $filter;
		$sqlstr .= " GROUP BY W.JKID ) CW  on cpjkd.JKID = CW.ID 
        LEFT OUTER JOIN  ( SELECT L_id,L_name AS ckmc  FROM location ) l ON cpjkd.l_id=l.l_id ,customer,location  where cpjkd.jkid>0 " . $filter;
	}
	//	return $sqlstr;
	//$query = mysql_query($sqlstr);
	return getjsonstoredata_new($sqlstr, 0);
...
function cpckdlist_pc()
{
	...
	if ($loc == "cpcksploc") {
		$sqlstr = "SELECT cpxsd.khid,cpxsd.khmc,cpckd.ckrq as czrq,xm.cdmc,xm.cpmc,xm.bzmc,xm.cpgg,xm.cpph,
			...
			FROM   CPxsdmx xm, cpckd,cpckdmx m,cpxsd,location,customer  
			WHERE cpxsd.xsid=xm.xsid and  cpckd.ckid = m.ckid 
			and   cpxsd.xsid=cpckd.xsid and xm.mxid=m.xsmxid  " . $filter;
	} else {
		...
		INNER JOIN cpxsd  ON (cpckd.xsid = cpxsd.xsid) ,location,customer  where cpckd.ckid>0 " . $filter;
		$sqlstr .= ") d LEFT OUTER JOIN  (
		SELECT  W.cKid AS id,SUM(w.cwccsl) AS ccsl, SUM(w.cwcczl) AS cczl,  SUM(w.ccje) AS ccje,  SUM(w.xjje) AS xjje  
		FROM CPCKDMX W,cpckd,cpxsd,location,customer  WHERE cpxsd.xsid=cpckd.xsid and cpckd.ckid=w.ckid  " . $filter;
...
function cpjcworklist_pc()
{
	...
	return getjsonstoredata_new($sqlstr1, 0);
}
...
WHERE  ( cpjxc.kcsl<>0 OR cpjxc.kczl<>0 )  " . $filter;
	...
	return getjsonstoredata_new($sql, 0);
}
function cpjcttlist_pc()
{
	...
	if (($jclb == "进仓") || ($jclb == "")) {
		...
		$filter .= " and customer.active=1 and location.active=1 ";
		$sqlstr1 = "SELECT '进仓' as jclb, cpjkd.khmc,commodity.S_name as cpmc, cpjkdmx.jcsl, cpjkdmx.jczl,0 as ccsl,0 as cczl, 0 as gfsl, 0 as gfzl, 0 as tzsl, 0 as tzzl FROM cpjkdmx INNER JOIN cpjkd ON (cpjkdmx.jkid = cpjkd.jkid) INNER JOIN Location ON (location.L_id = cpjkd.L_id) INNER JOIN Customer ON (Customer.C_id = cpjkd.khid) INNER JOIN commodity ON (cpjkdmx.cpid = commodity.S_id) where cpjkd.delbz=0  " . $filter;
	}
	if (($jclb == "出仓") || ($jclb == "")) {
		...
		$filter .= " and customer.active=1 and location.active=1 ";
		$sqlstr2 = "SELECT '出仓' as jclb,cpxsd.khmc,cpxsdmx.cpmc,0 as jcsl,0 as jczl,cpckdcw.sl as ccsl,cpckdcw.zl as cczl,0 as gfsl,0 as gfzl,0 as tzsl,0 as tzzl FROM cpckdmx INNER JOIN cpckd ON (cpckdmx.ckid = cpckd.ckid) INNER JOIN cpxsdmx ON (cpckdmx.xsmxid = cpxsdmx.mxid) INNER JOIN cpckdcw ON (cpckdcw.ckmxid = cpckdmx.ckmxid) INNER JOIN cpxsd ON (cpckd.xsid = cpxsd.xsid) INNER JOIN Location ON (location.L_id = cpxsd.L_id) INNER JOIN Customer ON (Customer.C_id = cpxsd.khid) where cpckd.delbz=0 " . $filter;
	}
	if (($sqlstr1 != "") && ($sqlstr2 != "")) {
		$sqlstr1 .= " union all " . $sqlstr2;
	} else {
	...
	if (($jclb == "过货") || ($jclb == "")) {
		...
		$filter .= " and customer.active=1 and location.active=1 ";
		$sqlstr0 = "SELECT '过货' as jclb,cpgfd.khmc,cpgfdmx.xmmc as cpmc,0 as jcsl,0 as jczl,0 as ccsl,0 as cczl,cpgfdmx.sl as gfsl,cpgfdmx.zl as gfzl,0 as tzsl, 0 as tzzl FROM cpgfdmx INNER JOIN cpgfd ON (cpgfdmx.gfid = cpgfd.gfid) INNER JOIN Location ON (location.L_id = cpgfd.L_id) INNER JOIN Customer ON (Customer.C_id = cpgfd.khid) where cpgfd.delbz=0  " . $filter;
	}
	if (($sqlstr1 != "") && ($sqlstr0 != "")) {
		$sqlstr1 .= " union all " . $sqlstr0;
	} else {
	...
	$filter .= " and customer.active=1 and location.active=1 ";
	$sqlstr0 = "SELECT '过车' as jclb,cpgfd.khmc,cpgfdmx.xmmc as cpmc,0 as jcsl,0 as jczl,0 as ccsl,0 as cczl,cpgfdmx.sl as gfsl,cpgfdmx.zl as gfzl,0 as tzsl, 0 as tzzl FROM wxcpgfdmx cpgfdmx INNER JOIN wxcpgfd cpgfd ON (cpgfdmx.gfid = cpgfd.gfid) INNER JOIN Location ON (location.L_id = cpgFd.L_id) INNER JOIN Customer ON (Customer.C_id = cpgFd.khid) where cpgfd.delbz=0 and cpgfd.fhbz>0" . $filter;
	if (($sqlstr1 != "") && ($sqlstr0 != "")) {
		$sqlstr1 .= " union all " . $sqlstr0;
	} else {
		if ($sqlstr0 != "") {
	...
	if (($jclb == "其它") || ($jclb == "")) {
		...
		$filter .= " and customer.active=1 and location.active=1 ";
		$sqlstr3 = "SELECT '其它' as jclb, cptzd.khmc,cptzdmx.cpmc, 0 as jcsl,0 as jczl,0 as ccsl,0 as cczl,0 as gfsl,0 as gfzl,cptzdmx.tzsl ,cptzdmx.tzzl FROM cptzd INNER JOIN cptzdmx ON (cptzd.tzid = cptzdmx.tzid) INNER JOIN Location ON (location.L_id = cptzd.L_id) INNER JOIN Customer ON (Customer.C_id = cptzd.khid) where cptzd.delbz=0 " . $filter;
	}
	if (($sqlstr1 != "") && ($sqlstr3 != "")) {
		$sqlstr1 .= " union all " . $sqlstr3;
	} else {
	...
	if (($jclb == "其它") || ($jclb == "")) {
		...
		$sqlstr3 = "SELECT '其它' as jclb,cptzd.newkhmc as khmc, cptzdmx.cpmc, 0 as jcsl, 0 as jczl, 0 as ccsl, 0 as cczl,0 as gfsl, 0 as gfzl,cptzdmx.tzsl,cptzdmx.tzzl FROM cptzd INNER JOIN cptzdmx ON (cptzd.tzid = cptzdmx.tzid)INNER JOIN Location ON (location.L_id = cptzd.L_id) INNER JOIN Customer ON (Customer.C_id = cptzd.khid) where cptzd.delbz=0 " . $filter;
	}
	if (($sqlstr1 != "") && ($sqlstr3 != "")) {
		$sqlstr1 .= " union all " . $sqlstr3;
	} else {
	...
	$filter .= " and customer.active=1 and location.active=1 ";
	$sqlstr2 = "SELECT '过户出仓' as jclb,wxcpghd.khmc,wxcpghdmx.cpmc,0 as jcsl,0 as jczl,wxcpghdcw.sl as ccsl,wxcpghdcw.zl as cczl,0 as gfsl,0 as gfzl,0 as tzsl,0 as tzzl FROM wxcpghdmx INNER JOIN wxcpghd ON (wxcpghdmx.ghid = wxcpghd.ghid) INNER JOIN Location ON (location.L_id = wxcpghd.L_id) INNER JOIN Customer ON (Customer.C_id = wxcpghd.khid) INNER JOIN wxcpghdcw ON (wxcpghdcw.mxid = wxcpghdmx.mxid)  where wxcpghd.delbz=0 " . $filter;
	if (($sqlstr1 != "") && ($sqlstr2 != "")) {
		$sqlstr1 .= " union all " . $sqlstr2;
	} else {
		if ($sqlstr2 != "") {
	...
	$filter .= " and customer.active=1 and location.active=1 ";
	$sqlstr2 = "SELECT '过户入仓' as jclb,customer.c_name AS khmc,wxcpghdmx.cpmc, wxcpghdcw.sl as jcsl, wxcpghdcw.zl as jczl, 0 as ccsl,0 as cczl, 0 as gfsl, 0 as gfzl, 0 as tzsl, 0 as tzzl FROM wxcpghdmx INNER JOIN wxcpghd ON (wxcpghdmx.ghid = wxcpghd.ghid) INNER JOIN Location ON (Location.l_id = wxcpghd.L_id) INNER JOIN wxcpghdcw ON (wxcpghdcw.mxid = wxcpghdmx.mxid) INNER JOIN customer ON (wxcpghd.newkhid = customer.c_id) where wxcpghd.delbz=0 " . $filter;
	if (($sqlstr1 != "") && ($sqlstr2 != "")) {
		$sqlstr1 .= " union all " . $sqlstr2;
	} else {
		if ($sqlstr2 != "") {
	...
	$cpbz = (int)$_GET["cpbz"];
	if (($khbz == 1) && ($cpbz == 0)) {
		$sql = "select khmc,sum(jcsl) as jcsl,sum(jczl) as jczl,sum(ccsl) as ccsl,sum(cczl) as cczl,sum(gfsl) as gfsl,sum(gfzl) as gfzl,sum(tzsl) as tzsl,sum(tzzl) as tzzl,sum(ccsl+jcsl+gfsl+tzsl) as sl,sum(cczl+jczl+gfzl+tzzl) as zl from (" . $sqlstr1 . ") k  group by khmc ";
	} else {
		if (($khbz == 0) && ($cpbz == 1)) {
			$sql = "select cpmc,sum(jcsl) as jcsl,sum(jczl) as jczl,sum(ccsl) as ccsl,sum(cczl) as cczl,
		sum(gfsl) as gfsl,sum(gfzl) as gfzl,sum(tzsl) as tzsl,sum(tzzl) as tzzl,sum(ccsl+jcsl+gfsl+tzsl) as sl,sum(cczl+jczl+gfzl+tzzl) as zl
		from (" . $sqlstr1 . ") k group by cpmc ";
		} else {
			$sql = "select khmc as bz,'' as  khmc, cpmc,sum(jcsl) as jcsl,sum(jczl) as jczl,sum(ccsl) as ccsl,sum(cczl) as cczl,sum(gfsl) as gfsl,sum(gfzl) as gfzl,sum(tzsl) as tzsl,sum(tzzl) as tzzl,sum(ccsl+jcsl+gfsl+tzsl) as sl,
		sum(cczl+jczl+gfzl+tzzl) as zl from (" . $sqlstr1 . ") k   group by khmc,cpmc ";
		}
	}
	//return $sql;
	//$query = mysql_query($sql);
...
function cpxsdmxlist_pc()
{
	...
	$sqlstr = " SELECT '1' as mxdh,m.kcid, m.xsid,m.mxid,m.cdmc,m.cpmc,m.bzmc,m.cpgg,m.jldw,
     	...
     	FROM cpxsdmx m,cpxsd,packing bz ,customer,location
		where cpxsd.xsid=m.xsid and bz.PS_id=m.bzid 
		and cpxsd.khid=customer.c_id and cpxsd.L_id=location.l_id and customer.active=1 and location.active=1 " . $filter;
	// return $sqlstr;
	//$query = mysql_query($sqlstr);
	return getjsonstoredata_new($sqlstr, 0);
}
function cpghdmxlist_pc()
{
	...
	$filter .= " and cpghd.khid=customer.c_id  AND cpghd.L_id=location.L_id and location.active=1 and customer.active=1 ";
	...
	$sqlstr = " SELECT '1' as mxdh,m.kcid, m.ghid,m.mxid,m.cdmc,m.cpmc,m.bzmc,m.cpgg,m.jldw,
     	...
     	bz.Quantity_Unit AS sldw,  bz.Weight_Unit AS zldw,m.sm,m.cpph,m.bzid   
     	FROM wxcpghdmx m,wxcpghd cpghd,packing bz,location,customer where cpghd.ghid=m.ghid and bz.PS_id=m.bzid " . $filter;
	//   return $sqlstr;
	//	$query = mysql_query($sqlstr);
	return getjsonstoredata_new($sqlstr, 0);
}
function wxcpgfdmxlist_pc()
{
	...
	$filter .= " and wxcpgfd.khid=customer.c_id  AND wxcpgfd.L_id=location.L_id and location.active=1 and customer.active=1 ";
	$sqlstr = "SELECT 
		'1' AS mxdh,
		m.*
	  FROM
		wxcpgfdmx m,
		wxcpgfd,customer,location
	  WHERE wxcpgfd.gfid = m.gfid 
		AND wxcpgfd.ztbz = 1 
		AND wxcpgfd.fhbz < 2 
		AND wxcpgfd.delbz = 0  " . $filter;
	$sqlstr = " SELECT  '1' as mxdd,m.*
	FROM wxcpgfdmx m,wxcpgfd,customer,location where wxcpgfd.gfid=m.gfid  " . $filter;
	//  return $sqlstr;
	//$query = mysql_query($sqlstr);
	return getjsonstoredata_new($sqlstr, 0);
}
function cpxsdlist_pc()
{
	...
	//		}
	$sqlstr = "SELECT cpxsd.*,cpxsd.xsid as id ,customer.shrs 
		FROM cpxsd ,customer,location  
		where cpxsd.khid=customer.c_id and cpxsd.xsid>0 and cpxsd.L_id=location.L_id  and location.active=1 and customer.active=1 " . $filter;
	//	return $sqlstr ; 
	//$query = mysql_query($sqlstr);
	return getjsonstoredata_new($sqlstr, 0);
}
function cpghdlist_pc()
{
	...
	if ($loc == "cpghsploc") {
		...
		$filter .= " and location.active=1 and customer.active=1 ";
		if ($_GET["startdate"]) {
			$filter .= " and cpghd.ghrq>='" . $_GET["startdate"] . "'";
		}
		if ($_GET["enddate"]) {
	...
	location.l_name AS ckmc ,customer.shrs
    FROM  wxcpghdmx cpghdmx,wxcpghdcw W, wxcpghd cpghd,location,customer   
    WHERE cpghd.ghid = cpghdmx.ghid and cpghdmx.mxid=w.mxid and cpghd.khid=customer.c_id 
	AND cpghd.L_id=location.L_id " . $filter;
		$filter = " and cpghd.ztbz>0 and  cpghd.delbz=0 and cpghd.fhbz>0  ";
		if ($_GET["startdate"]) {
			$filter .= " and cpghd.ghrq>='" . $_GET["startdate"] . "'";
	...
	cpghd.ghdh  as dh,0 as sl,0 as zl,w.sl as jcsl  ,w.zl as jczl ,location.l_name AS ckmc,customer.shrs
    FROM  wxcpghdmx cpghdmx ,wxcpghdcw W, wxcpghd  cpghd,location,customer  
    WHERE cpghd.ghid = cpghdmx.ghid and cpghdmx.mxid=w.mxid and customer.c_id=cpghd.khid AND cpghd.L_id=location.L_id 
	and location.active=1 and customer.active=1 " . $filter;
		$sqlstr =	$sqlstr1 . " union all " .	$sqlstr2;
	} else {
		//		 $sqlstr = " SELECT cptzd.*,cptzd.tzid as id,Location.L_name as ckmc FROM cptzd,Location where location.L_id=cptzd.L_id ".$filter;
		$sqlstr = "SELECT cpghd.*,ghid as id,customer.shrs FROM wxcpghd cpghd,customer,location  
		 where location.l_id=cpghd.L_id and location.active=1 and customer.active=1 and customer.c_id=cpghd.khid  " . $filter;
	}
	//	return $sqlstr ; 
	//$query = mysql_query($sqlstr);
	return getjsonstoredata_new($sqlstr, 0);
...
function wxcpgfdlist_pc()
{
	...
	//  $filter .= " and location.active=1 and customer.active=1 ";
	$sqlstr = " SELECT wxCPgfD.*,l.ckmc,wxcpgfd.gfid as id,customer.shrs 
		 FROM  wxcpgfd  INNER JOIN customer ON customer.c_id=wxcpgfd.khid 
		 INNER JOIN Location ON Location.L_id=wxcpgfd.L_id ";
	$sqlstr .= " LEFT OUTER JOIN  ( SELECT L_id,L_name AS ckmc  FROM location ) l ON wxcpgfd.l_id=l.l_id 
		 where location.active=1 and customer.active=1 " . $filter;
	/*$sqlstr = " SELECT wxCPgfD.*,l.ckmc,wxcpgfd.gfid as id,customer.shrs 
		 FROM  wxcpgfd  INNER JOIN customer ON customer.c_id=wxcpgfd.khid " ;
        $sqlstr .= " LEFT OUTER JOIN  ( SELECT L_id,L_name AS ckmc  FROM location ) l ON wxcpgfd.l_id=l.l_id ,customer,location  where wxcpgfd.gfid>0 ". $filter ;
*/
	// 	return $sqlstr . $loc; 
	//	$query = mysql_query($sqlstr);
	return getjsonstoredata_new($sqlstr, 0);
...
function getstoredata($query, $total)
{ //返回STORE所需的数据
...
function getjsonstoredata_new($sqlcmd, $total)
{ //返回STORE所需的数据
	//mysql_num_fields($query)字段数
	//mysql_field_name($query,0) 字段名称
	//mysql_numrows($query)记录数
	...
	if ($query) {
		...
		$arr['success'] = false;
		$arr['data'] = array('id' => 1, 'msg' => urlencode('数据查询操作失败！'), 'sqlcmd' => $sqlcmd);
	}
	return urldecode(json_encode($arr));
	//return json_encode($arr);
}
function currencylist1()
{
	...
	//$query = mysql_query($sqlstr);
	return getstoredata_new($sqlstr, 0);
}
function menulist()
{
	$bz = $_GET['bz'];
...
function returnjsonokdata($query)
{ //返回json格式的数据
	if ($query) {
		$arr['success'] = true;
		$arr['data'] = 'ok';
...
function cpgfkdmxsave()
{
	...
	if ($loc == 'delete') {
		$gfid = $_GET['data'];
		$cpgfdstr = " update  wxcpgfd  set  del='" . $czy . "',delrq=now(),delbz=1 where delbz=0 and gfid=" . $gfid;
		mysql_query($cpgfdstr);
		if (mysql_errno() > 0) {
			return '{result:"fail",msg:"数据保存失败!" }';
			//	return '数据删除失败!!!'.$cpgfdstr;
	...
	if ($loc == 'ok') {
		$username = $o['username'];
		$data = json_decode(json_decode(base64_decode($_GET['data'])), true);
		$gfid = $data['gfid'];
		$ztbz = $data['ztbz'];
		$shr = $data['shr'];
		$cpgfdstr = " update  wxcpgfd  set  khshr='" . $shr . "',khshrq=now(),ztbz=" . $ztbz . "  where delbz=0 and ztbz=0 and gfid=" . $gfid;
		mysql_query($cpgfdstr);
		if (mysql_errno() > 0) {
			return '{result:"fail",msg:"数据保存失败!" }';
			//return '数据删除失败!!!'.$cpgfdstr;
		} else {
			return '{result:"success",zt:"' . $ztbz . '",gfid:' . $gfid . ' }';
		}
	}
	$cpgfdmx = $o['gfdmx'];
	...
	$arr = mysql_fetch_assoc($result);
	$dh = $arr['dh'];
	$cpgfdstr = " insert into wxcpgfd (gfdh,L_id,khid,khmc,ckmc,cphm,area,sfr,czy,cnote,khsl,khzl,kdrq,endrq)values('";
	$cpgfdstr .= $dh . "'";
	$cpgfdstr .= "," . $o['ckid'];
	$cpgfdstr .= "," . $o['khid'];
	$cpgfdstr .= ",'" . $o['khmc'] . "'";
...
function cpxsdshsave()
{
	...
	if ($loc == "ok") {
		$shr = $data['shr'];
		$ztbz = $data['ztbz'];
		$sqlstr = " update cpxsd set ztbz=" . $ztbz . ",shrq=NOW(), shr='" . $shr . "' where  ztbz=0 and delbz=0 and xsid=" . $xsid;
	} else {
		$xsid = $_GET['data'];
		$ztbz = "";
		if ($loc == "lastdel") {
			$sqlstr = " update cpxsd set delbz=1,delrq=NOW(), del='" . $username . "' where  ztbz>0 and delbz=0 and xsid=" . $xsid;
		} else {
			$sqlstr = " update cpxsd set delbz=1,delrq=NOW(), del='" . $username . "' where  ztbz=0 and delbz=0 and xsid=" . $xsid;
		}
	}
...
function cpghdshsave()
{
	...
	$data = json_decode(json_decode(base64_decode($_GET['data'])), true);
	$xsid = $data['ghid'];
	//	return $xsid.'   '.$data['shr'].'===='.$data['ztbz'];
	if ($loc == "ok") {
		$ztbz = $data['ztbz'];
		$shr = $data['shr'];
		$sqlstr = " update wxcpghd set ztbz=" . $ztbz . ",khshrq=NOW(), khshr='" . $shr . "' where  ztbz=0 and delbz=0 and ghid=" . $xsid;
	} else {
		$xsid = $_GET['data'];
		if ($loc == "lastdel") {
			$sqlstr = " update wxcpghd set delbz=1,delrq=NOW(), del='" . $username . "' where  ztbz>0 and delbz=0 and ghid=" . $xsid;
		} else {
			$sqlstr = " update wxcpghd set delbz=1,delrq=NOW(), del='" . $username . "' where  ztbz=0 and delbz=0 and ghid=" . $xsid;
		}
	}
	...
	foreach ($params as $arr) {
		...
		switch ($optype) {
			case 1:
				...
				$sql = "insert into Customer(C_code,L_id,shrs,C_name,Address,Tel,smsphone,Py_code,password,C_shortname,Aloneprice,Active) values('" . $arr['C_code'] . "'";
				$sql .= "," . $arr['L_id'];
				$sql .= "," . $arr['shrs'];
				$sql .= ",'" . $arr['C_name'] . "'";
				$sql .= ",'" . $arr['Address'] . "'";
